...
# === 1. Load image data ===
img_paths = sorted(
    glob.glob("/home/ridge/RMIT/Rover/RoverTeam-GUI/webrtc-gui/Model-Training/extracted_images/**/*.png", recursive=True)
)
...
# === precompute for global distance scale ===
X_scaled_all = np.vstack((X_train_scaled, X_test_scaled))
global_max_dist = np.max(euclidean_distances(X_scaled_all, X_scaled_all))
...
def compare_to_screenshot(screenshot_path):
    ...
    # Option A: Linear scaling to 0‑100 using global max
    # similarities = (1 - distances / global_max_dist) * 100

    # Option B: Exponential similarity (better spread)
    k = 1000.0  # adjust this to control fall‑off
    similarities = np.exp(-distances / k) * 100
